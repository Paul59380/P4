...
foreach ($commentSigned as $comment) {
    ...
    <fieldset id="reported_comment">
        <legend>De : <strong style="color:red"><?= htmlspecialchars($comment->getUser()->getPseudo()) ?> </strong>
            Posté le : <strong><?= htmlspecialchars($comment->getReportingDate()) ?></strong></legend>
        <br/>
        ...
        <a title="Modifier" href="index.php?action=updateComment&id=<?= $comment->getId() ?>">
            <i style="color: #288ba3; padding-left: 10px" class="fas fa-pen-fancy"></i></a>
        <a title="Supprimer" href="index.php?action=deleteComment&id=<?= $comment->getId() ?>&origin=<?= htmlspecialchars($comment->getIdComment()) ?>">
            <i style="color: red; padding-left: 10px" class="fas fa-times-circle fa-1x"></i></a>
        <a title="Valider" href="index.php?action=validComment&id=<?= $comment->getId() ?>">
